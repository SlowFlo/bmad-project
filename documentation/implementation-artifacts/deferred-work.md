...
- source_spec: `documentation/implementation-artifacts/spec-2-1-chercher-des-candidats-du-niveau-exact.md`
  ...
  resolution: Clos par l'ajout d'une affectation annotée `port: PortVivier = depot` dans `test_le_depot_satisfait_le_port_du_vivier`, une fois mypy installé. Quatre sondes disent ce qu'elle tient exactement, plutôt que de le supposer : un paramètre **supprimé** de `profils_du_sport` fait échouer mypy (22 erreurs), un **type de retour** changé aussi — et l'entrée avait raison de dire qu'`isinstance` n'y voyait rien. Deux nuances retenues : un paramètre **renommé** passe (mypy ne compare pas les noms ici, et le domaine appelle positionnellement), et un port qui se mettrait à **filtrer** ne change aucune signature — c'est le test de comportement `test_le_port_rend_un_niveau_inconnu_que_le_domaine_ecarte` qui le retient, et lui seul.
